...
# 폰트 경로 지정
FONT_PATH = "Maplestory Light.ttf"  # 폰트 파일을 실행 파일과 같은 경로에 두거나 절대 경로로 설정
...
def recognize_and_track_faces(frame, target_encodings, tracking, user_name, threshold=0.39):
    ...
    return tracking, face_recognized, frame
...
def main():
    ...
    while True:
        ...
        if mode == 'face_recognition':
            tracking, face_recognized, frame = recognize_and_track_faces(frame, target_encodings, tracking, user_name)
